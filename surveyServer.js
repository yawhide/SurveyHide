	...
	app.post('/editSurvey', function (req, res, next) {
		passport.authenticate('local', function (err, user, info) {
			...
			if(typeof req.body.radio == 'undefined'){
				...
				res.redirect('/survey/' + req.body.radio);
			}
			else if(req.body.edit){
				res.redirect('/new-survey/' + req.body.radio);
			}
			else if(req.body.stat){
				res.redirect('/stats/' + req.body.radio);
			}
			else if(req.body.preview){
				res.redirect('/preview/' + req.body.radio);
			}
			else if(req.body.deleteFlag){
				res.redirect('/delete/' + req.body.radio);
			}
		})(req, res, next);
	});
